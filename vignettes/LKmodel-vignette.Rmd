...
library(LKmodel)
```
```{r results='asis', echo=FALSE, fig.show='hold',fig.align="center",fig.width=6,fig.height=6,fig.cap="検出力曲線による検定法の比較．有意水準は両側$p=0.05$で，プラセボ群の$B$は0.85，$Q$は0.75とし，実薬群の$B$は0.9，$Q$は0.9とした．"}
###################### END OF PROGRAM #######################
...
for (cond in 1:3){
  ...
  for (eachn in seq(50,100,5)){
    ...
    for (n in 1:N){
      ...
      #browser()
      cpval <- c(cpval, LKmodel.test(mdata,mdatp,method=para[cond])$p.value)
    }
...
家庭数は，すべて4人家庭を想定した．
```{r results='asis', echo=FALSE, fig.show='hold',fig.align="center",fig.width=6,fig.height=6,fig.cap="実薬群のQを変化させた際の検出力曲線．有意水準は両側p=0.05で，プラセボ群のQは0.75とした．"}
