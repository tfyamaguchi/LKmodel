...
library(doParallel)
```
```{r eval=TRUE, results='asis', echo=FALSE, fig.show='hold',fig.align="center",fig.width=6,fig.height=6,fig.cap="検出力曲線による検定法の比較．有意水準は両側$p=0.05$で，プラセボ群の$B$は0.85，$Q$は0.75とし，実薬群の$B$は0.9，$Q$は0.9とした．"}
###################### END OF PROGRAM #######################
...
ccpval <- foreach (eachn=seq(50,100,5), .combine = rbind) %dopar% {
    #cat("N=",eachn,"\n")
...
家庭数は，すべて4人家庭を想定した．
```{r eval=TRUE, results='asis', echo=FALSE, fig.show='hold',fig.align="center",fig.width=6,fig.height=6,fig.cap="実薬群のQを変化させた際の検出力曲線．有意水準は両側p=0.05で，プラセボ群のQは0.75とした．"}
...
for (cond in 1:4){
  ...
  ccpval <- NULL
  registerDoParallel(detectCores())
  ccpval <- foreach (eachn=seq(20,160,20), .combine = rbind) %dopar% {
    #cat("N=",eachn,"\n")
    cpval <- NULL
    for (n in 1:N){
      cda <- cdp <- NULL
